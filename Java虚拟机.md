- [内存模型](#内存模型)
  ...
  - [JVM内存模型里的堆和栈有什么区别？](#jvm内存模型里的堆和栈有什么区别)
  - [栈中存的到底是指针还是对象？](#栈中存的到底是指针还是对象)
  - [堆分为哪几部分呢？](#堆分为哪几部分呢)
  - [如果有个大对象一般是在哪个区域？](#如果有个大对象一般是在哪个区域)
  - [方法区中的方法的执行过程？](#方法区中的方法的执行过程)
  - [方法区中还有哪些东西？](#方法区中还有哪些东西)
  - [String保存在哪里呢？](#string保存在哪里呢)
  - [String s = new String（“abc”）执行过程中分别对应哪些内存区域？](#string-s--new-stringabc执行过程中分别对应哪些内存区域)
  - [引用类型有哪些？有什么区别？](#引用类型有哪些有什么区别)
  - [弱引用了解吗?举例说明在哪里可以用?](#弱引用了解吗举例说明在哪里可以用)
  - [内存泄漏和内存溢出的理解？](#内存泄漏和内存溢出的理解)
  - [jvm 内存结构有哪几种内存溢出的情况？](#jvm-内存结构有哪几种内存溢出的情况)
  - [遇到过堆溢出的情况吗？如何解决？](#遇到过堆溢出的情况吗如何解决)
  - [栈溢出的情况呢？](#栈溢出的情况呢)
- [类初始化和类加载](#类初始化和类加载)
  - [创建对象的过程？](#创建对象的过程)
  - [对象的生命周期](#对象的生命周期)
  - [类加载器有哪些？](#类加载器有哪些)
  - [Java 中 双亲委派 是什么？有啥用？](#java-中-双亲委派-是什么有啥用)
  - [双亲委派模型的作用](#双亲委派模型的作用)
  - [讲一下类加载过程？](#讲一下类加载过程)
- [垃圾回收](#垃圾回收)
  - [什么是Java里的垃圾回收？如何触发垃圾回收？](#什么是java里的垃圾回收如何触发垃圾回收)
  - [判断垃圾的方法有哪些](#判断垃圾的方法有哪些)
  - [垃圾回收算法是什么，是为了解决了什么问题？](#垃圾回收算法是什么是为了解决了什么问题)
  - [垃圾回收器有哪些？](#垃圾回收器有哪些)
  - [标记清除算法的缺点是什么？](#标记清除算法的缺点是什么)
  - [什么是 Stop-The-World（STW）？](#什么是-stop-the-worldstw)
  - [垃圾回收的哪些阶段会发生 STW？](#垃圾回收的哪些阶段会发生-stw)
  - [1. **Serial / Parallel 收集器（传统分代收集器）**](#1-serial--parallel-收集器传统分代收集器)
  - [2. **CMS 收集器（Concurrent Mark Sweep）**](#2-cms-收集器concurrent-mark-sweep)
  - [3. **G1 收集器（Garbage-First）**](#3-g1-收集器garbage-first)
  - [4. **ZGC / Shenandoah 收集器（低延迟收集器）**](#4-zgc--shenandoah-收集器低延迟收集器)
  - [5. **安全点（Safepoint）引起的 STW**](#5-安全点safepoint引起的-stw)
  - [minorGC、majorGC、fullGC的区别，什么场景触发full GC](#minorgcmajorgcfullgc的区别什么场景触发full-gc)
  - [垃圾回收器 CMS 和 G1的区别？](#垃圾回收器-cms-和-g1的区别)
  - [什么情况下使用CMS，什么情况使用G1?](#什么情况下使用cms什么情况使用g1)
  - [G1回收器的特色是什么？](#g1回收器的特色是什么)
...
- **保证安全性**：由于Java核心库被启动类加载器加载，而**启动类加载器只加载信任的类路径中的类**，这样可以防止不可信的类假冒核心类，增强了系统的安全性。例如，恶意代码无法自定义一个Java.lang.System类并加载到JVM中，因为这个请求会被委托给启动类加载器，而启动类加载器只会加载标准的Java库中的类。
...
- 连接：验证、准备、解析 3 个阶段统称为连接。
  - **验证**：确保class文件中的字节流包含的信息，**符合当前虚拟机的规范**，保证这个被加载的class类的正确性，不会危害到虚拟机的安全。验证阶段大致会完成以下四个阶段的检验动作：文件格式校验、元数据验证、字节码验证、符号引用验证
  - **准备**：为**类中的静态字段分配内存**，并设置默认的初始值，比如int类型初始值是0。被final修饰的static字段不会设置，因为final在编译的时候就分配了
...
主要依据两种主流的垃圾回收算法来实现：**引用计数法和可达性分析算法**。
...
- **原理**：**如果一个对象到GC Roots没有任何引用链相连，那么这个对象就被认为是不可达的，可以被回收。**GC Roots对象包括：虚拟机栈（栈帧中的本地变量表）中引用的对象、方法区中类静态属性引用的对象、本地方法栈中JNI（Java Native Interface）引用的对象、活跃线程的引用等。

### 垃圾回收算法是什么，是为了解决了什么问题？

垃圾回收（Garbage Collection，GC）算法是 JVM 自动管理内存的一种机制，用于**自动识别和回收不再被程序使用的对象所占用的内存空间**。它代替了开发者手动释放内存的操作，是 Java 实现“自动内存管理”的核心技术。

1. **解决手动内存管理的弊端**

   - **避免内存泄漏**：在 C/C++ 中，开发者需手动调用 `free` 或 `delete` 释放内存，若忘记释放，对象占用的内存将无法回收，导致内存泄漏，程序运行越久占用内存越大，最终崩溃。

   - **杜绝悬垂指针**：若内存释放后仍有指针指向该区域，后续访问将导致非法操作或程序崩溃。

   - **防止重复释放**：同一内存被释放两次可能引发运行时错误。

2. **提高开发效率**
   - 开发者无需关注内存释放时机，可专注于业务逻辑，降低编程复杂度，提升开发效率。

3. **保证程序稳定性**
   - 自动内存管理避免了因手动操作失误导致的内存错误，使程序在有限的内存中稳定运行，降低内存溢出（OOM）的风险。

4. **实现内存动态管理**
   - 垃圾回收器可根据程序运行状况动态调整内存分配和回收策略，更好地适应不同负载场景。
...
1）**标记-清除**：先标记所有可回收对象，然后直接清理它们的内存。但这样会产生很多内存碎片，后续分配大对象时可能找不到连续空间。

![image.png](https://pic.code-nav.cn/mianshiya/question_picture/1783388929455529986/jjsNMPkI_image_mianshiya.webp)

2）**复制算法**：把内存分成两块，每次只用其中一块，回收时把存活对象复制到另一块，然后清空当前块。这种方式没有碎片，但会浪费一半内存，适合存活对象少的场景（比如新生代）。

![image.png](https://pic.code-nav.cn/mianshiya/question_picture/1783388929455529986/0bvKS4BK_image_mianshiya.webp)

3）**标记-整理**：老年代常用。老年代对象活得久，用复制算法得复制一大堆太慢，用标记-清除又有碎片。标记后不是直接清除，而是把存活对象往一端移动，然后清理边界外的内存，这样内存更整齐，但移动对象会有性能开销。

![image.png](https://pic.code-nav.cn/mianshiya/question_picture/1783388929455529986/lZN5pYSy_image_mianshiya.webp)
...
**Stop-The-World** 是指 Java 虚拟机（JVM）在执行垃圾回收（GC）时，**暂停所有应用线程的执行**，只允许垃圾回收线程运行的阶段。

- **目的**：在垃圾回收过程中，避免应用线程同时修改对象引用，导致 GC 无法准确标记存活对象或出现对象状态不一致。
- **影响**：STW 期间，应用无法处理用户请求，导致系统停顿（卡顿）。停顿时间越短，对用户体验影响越小。

### 垃圾回收的哪些阶段会发生 STW？

不同垃圾收集器的设计不同，STW 的阶段和时长也各异。以下是主流收集器的 STW 阶段分析：

### 1. **Serial / Parallel 收集器（传统分代收集器）**

- **Minor GC（年轻代 GC）**：**完全 STW**。复制存活对象到 Survivor 或老年代时，必须暂停应用线程。
- **Major GC / Full GC（老年代 GC）**：**完全 STW**。标记-整理或标记-清除过程全部暂停应用。

### 2. **CMS 收集器（Concurrent Mark Sweep）**

- **初始标记（Initial Mark）**：**STW**（但时间很短）。标记 GC Roots 直接关联的对象。
- **并发标记（Concurrent Mark）**：**无 STW**。与应用线程并发执行。
- **重新标记（Remark）**：**STW**（时间稍长）。修正并发标记期间因应用线程运行导致变动的对象标记。
- **并发清除（Concurrent Sweep）**：**无 STW**。与应用线程并发清除垃圾对象。

### 3. **G1 收集器（Garbage-First）**

- **年轻代收集（Young GC）**：**STW**。暂停应用线程，将 Eden 区存活对象复制到 Survivor 或老年代。
- **初始标记（Initial Mark）**：**STW**（通常借助 Young GC 完成，时间很短）。
- **并发标记（Concurrent Marking）**：**无 STW**。与应用线程并发进行全局标记。
- **最终标记（Final Marking）**：**STW**（时间较短）。处理并发标记阶段的 SATB 缓冲区。
- **混合收集（Mixed GC）**：**STW**。回收部分老年代 Region。

### 4. **ZGC / Shenandoah 收集器（低延迟收集器）**

- **几乎所有的 GC 操作都并发进行**，STW 时间极短（通常在 10ms 以内）。
- **短暂 STW 阶段**：如 ZGC 的并发根标记、并发引用处理等过程中，仅有非常短暂的暂停（如安全点同步）。

### 5. **安全点（Safepoint）引起的 STW**

- 除了明确的 GC 阶段，JVM 在**安全点**也可能暂停所有线程（例如：代码反优化、偏向锁撤销、线程 dump 等）。GC 通常也需要线程到达安全点才能开始。
...
- **CMS适用场景：**
  - **低延迟需求**：适用于对响应时间敏感的应用程序，通过并发标记和清除减少用户线程暂停。
  - **老年代收集**：专注于老年代的垃圾回收，需配合ParNew等年轻代收集器使用。
  - **小堆内存**：适合堆内存较小（通常4GB-6GB以下）的环境，管理开销低且碎片问题不突出。
  - **CPU资源有限**：并发阶段对CPU占用较少，适合核心数不多的服务器。
  - **JDK 8老系统**：已稳定运行且经过充分调优的遗留系统，无需升级即可维持现状。
- **G1适用场景：**
  - **大堆内存**：适用于需要管理数GB乃至上百GB堆内存的场景，通过Region分区降低扫描开销。
  - **可预测停顿**：支持通过`-XX:MaxGCPauseMillis`设定停顿时间目标，适合有严格SLA要求的服务。
  - **内存碎片敏感**：采用复制和标记-整理算法，消除内存碎片，提高内存利用率。
  - **动态负载适应**：能根据对象分配速率和存活情况自动调整回收策略，适应突发流量。
  - **现代多核环境**：利用多CPU资源进行并发标记和混合回收，平衡延迟与吞吐量。
